...
from mood import config as cfg
from mood import EarlyStopping
from mood import LRScheduler
...
train_classes = [label for _, label in tqdm(train_data, desc="Train Classes")]
...
for idx, (image, label) in tqdm(enumerate(train_data), desc="Sample weight initialize"):
...
for epoch in trange(0, cfg.NUM_OF_EPOCHS):
    ...
    for (data, target) in tqdm(train_dataloader, desc="Training", leave=False):
    ...
    with torch.set_grad_enabled(False):
        ...
        for (data, target) in tqdm(val_dataloader, desc="Validation", leave=False):
